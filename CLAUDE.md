...
## Where the project stands (2026-07-06)

**v1.0.0 — feature-complete, packaged, ready for store launch.** All 10 brief
...
1. Set signing env vars (`SPASYNTH_CODESIGN_IDENTITY`,
   ...
   re-run `./scripts/build_release.sh` for a signed/notarized pkg.
2. Download the `spasynth-installer-Windows` CI artifact into both
   `dist/shopify/` folders.
...
- `installers/` — macOS pkg builder (signing hooks), Windows Inno `.iss`.
