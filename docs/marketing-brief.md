...
factually accurate to the shipping build; do not invent features or
...
- **Probability controls:** CHANCE (steps fire or rest), STUTTER (ratchet
  rolls of 2 to 4 repeats), JUMP (random octave leaps), HUMAN (per-hit velocity
  spread). Patterns become performances.

### Playability
- Global glide/portamento: Off / Always / Legato modes, 1 ms to 2 s, applied
...
- Presets use portable paths, so sessions and presets survive the library
  living anywhere.
...
- FX chain with live scopes: distortion, chorus, sync delay (ping-pong),
  reverb, 3-band EQ
- 270 factory presets generated from the Silverplatter library (Keys /
